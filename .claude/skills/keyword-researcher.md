...
```
seo/prompts/keyword-researcher.md
```
...
All paths are relative to `<VitalCore project root>/seo/`. Always cd there or use absolute paths.
...
5. **Tell the user where the dashboard is** and offer to open it:
   ...
   Open with: open seo/output/keywords/dashboard.html
   ```
...
```
seo/launchd/com.example.seo-keyword-researcher.plist
```
